...
fn translate_flow(
    ...
    match flow {
        ...
        Flow::Switch {
            ...
            // Helper: open nested case blocks, emit br_table once all targets are open,
            // then translate each case body after its label.
            fn open_cases(
                builder: &mut InstrSeqBuilder,
                module: &mut Module,
                ctx: &mut TranslateFlowContext,
                functions_to_link: &mut HashSet<FunctionId>,
                cases: &[&Flow], // in reverse nesting order
                ...
                if case_index == cases.len() {
                    ...
                    // Unreachable code is reached when the index is out of range
                    builder.unreachable();
