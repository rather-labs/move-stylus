...
mod bytes_type {
    ...
    sol!(
        ...
        function testDynamicBytes(bytes data) external returns (uint8[]);
    );
    ...
    #[case(testDynamicBytesCall::new((bytes!("42434445"),)), vec![0x42, 0x43, 0x44, 0x45])]
    fn test_bytes_as_vec<T: SolCall, V: SolValue>(
        #[by_ref]
