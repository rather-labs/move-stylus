...
entry fun deref_arg(y: &vector<u8>): vector<u8> {
  *y
}

entry fun identity_vec_ref(x: &vector<u8>): &vector<u8> {
    x
}

entry fun call_deref_arg(x: vector<u8>): vector<u8> {
  deref_arg(&x)
}

entry fun dummy(_v: &vector<u8>) {
    // Does nothing, but forces a borrow
}

entry fun call_dummy(v: vector<u8>) {
    dummy(&v); 
}

entry fun vec_from_element(index: u64): vector<u8> {
    let v = vector[10u8, 20u8];
    let x = v[index];  
    vector[x]
}

entry fun get_element_vector(index: u64): vector<u8> {
    let v = vector[vector[10u8, 20u8], vector[30u8, 40u8]];
    let x = v[index];  
    x
}

entry fun deref_mut_arg(x: &mut vector<u8> ): vector<u8> {
 *x
}

entry fun write_mut_ref(x: &mut vector<u8> ): vector<u8> {
 *x = vector<u8>[1, 2, 3];
 *x 
}
...
entry fun miscellaneous_2(): vector<u8> {
 let mut x = vector<u8>[1, 2, 3];
 let y =  x;
 x = vector<u8>[4, 5, 6];
 let w = x;
...
entry fun miscellaneous_3(x: vector<u8>): vector<u8> {
  let mut y = x;
  let a = &mut y[0];
  let b = *a;
  *a = 99;
...
entry fun miscellaneous_4(): vector<u8> {
...
entry fun miscellaneous_5(): vector<u8> {
  let mut x = vector[vector[1u8, 2u8], vector[3u8, 4u8]]; // x = [ [1, 2], [3, 4] ]
  let a = &mut x[0]; // a = vector[1, 2]
  *vector::borrow_mut(a, 1) = 12; // a = vector[1, 12]
  let b = *a; // b = vector[1, 12]
...
entry fun freeze_ref(y: vector<u8>): vector<u8> {
    let mut x = vector<u8>[1, 2, 3];
