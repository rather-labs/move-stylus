...
3.  The `Wrapper` is transferred to the sender. The inner `Object` is no longer a top-level asset; it is now part of the `Wrapper`'s private state and is not directly accessible from global storage.
